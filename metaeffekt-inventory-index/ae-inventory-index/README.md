...
- use of indexes on UIDs and hashes to increase query performance
...
datasets only once in the database. Further information is provided in [Persistence](./persistence/README.md).
...
information is provided in [Persistence](./persistence/README.md).
...
contain the most data and normal views then are used to join them and build the final view for the query. More information is provided
in [Views](./views/README.md).

## Indexes

To increase performance and achieve fast access to the tables when building joins or querying the linking tables and datasets, on every dataset UID,
referenced UIDs in links and hash columns in tables an index is set.xs